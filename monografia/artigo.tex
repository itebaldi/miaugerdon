...
\begin{abstract}
\textbf{Introduction:} Perceptions of an unfamiliar person can be shaped by the information received about them, and misinformation may involve not only false content but also generalization, repetition and the delegitimization of sources.
...
O artigo está organizado da seguinte forma: a Seção 2 apresenta a fundamentação teórica e os trabalhos relacionados; a Seção 3 descreve a concepção de \textit{Miaugerdon}; a Seção 4 discute suas escolhas narrativas e mecânicas e apresenta as limitações do trabalho; e a Seção 5 reúne as conclusões e os trabalhos futuros.
...
Os serious games são jogos desenvolvidos com objetivos que ultrapassam o entretenimento, podendo ser utilizados em contextos de aprendizagem, conscientização e transformação social. A revisão realizada por Dallaqua e Carvalho mostra que esse campo reúne trabalhos voltados ao uso de jogos para abordar problemas sociais, explorar valores e estimular reflexão por meio de experiências interativas \cite{dallaqua}. Nesses jogos, a compreensão do tema pode surgir não apenas da apresentação de informações, mas também das situações, escolhas e consequências vivenciadas pelo jogador.
...
De forma mais direta, Martins et al. apresentam \textit{Typical Day}, uma experiência desenvolvida para representar situações cotidianas enfrentadas por imigrantes em Portugal, incluindo episódios de discriminação e xenofobia \cite{typicalday}. O jogo utiliza essas situações para aproximar jogadores não imigrantes das dificuldades vivenciadas por esse grupo e estimular reflexão sobre experiências que podem não ser reconhecidas como formas de preconceito.

Esses trabalhos reforçam a importância do contato e da tomada de perspectiva na revisão de julgamentos sobre grupos percebidos como diferentes, além de apontarem o potencial dos jogos como espaços para representar e explorar preconceito, alteridade e exclusão social de forma interativa.
...
Embora Soneca não represente diretamente um grupo social específico, essa dinâmica pode remeter a diferentes formas de exclusão social, entre elas a xenofobia, na qual indivíduos percebidos como externos ao grupo podem ser associados a estereótipos e características negativas antes do contato direto. Essa forma de construção do ``outro'' também aparece em jogos que abordam experiências de migração e discriminação, como \textit{Typical Day} \cite{typicalday}. A opção por não vincular Soneca a um grupo específico, entretanto, preserva a alegoria do ``outro'' e permite que a experiência seja interpretada a partir de diferentes contextos de alteridade.
